...
async function getRandomSnode(excludingEd25519Snode?: Array<string>): Promise<Snode> {
  window?.log?.debug(`Building routing path using pool of ${randomSnodePool.length} nodes`);
...
async function TEST_fetchFromSeedWithRetriesAndWriteToDb() {
  ...
  window?.log?.debug('Fetching initial relay nodes from seed:', JSON.stringify(seedNodes));
